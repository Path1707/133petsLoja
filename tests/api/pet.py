...
def testar_incluir_pet():
    ...
    nome_pet_esperado = 'Bolt'
    tag_esperada = 'Vacinado'

    # Executa
    resultado_obtido = requests.post(url=base_url + '/pet',
                                     data=open('C:\\Users\\pathm\\PycharmProjects\\133pets\\vendors\\json\\pet1.json',
                                               'rb'),
                                     headers=headers
                                     )
    # Valida
...
def testar_alterar_pet():
    ...
    resultado_obtido = requests.put(
        url=f'{base_url}/pet/',
        data=open('C:\\Users\\pathm\\PycharmProjects\\133pets\\vendors\\json\\pet2.json', 'rb'),
        headers=headers
    )
